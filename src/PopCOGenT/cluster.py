...
def main():
    ...
    parser.add_argument(
        "--infomap_path", type=str, default=None, help="Path to infomap binary"
    )
    ...
    initial_edgefile = "infomap_out/{base}_{mindiv}.txt".format(
        base=outfile_base, mindiv=str(clonal_cutoff)
    )
    cluster_file = "{output_dir}/{initial_edgefile}.cluster.tab.txt".format(
        initial_edgefile=os.path.basename(initial_edgefile), output_dir=output_dir
    )
    graphml_unclust_name = "{output_dir}/{initial_edgefile}.unclust.graphml".format(
        initial_edgefile=os.path.basename(initial_edgefile), output_dir=output_dir
    )
    ...
    for i, component in enumerate(nx.connected_component_subgraphs(G_unclust)):
        ...
        if (len(component.nodes())) > 1:
            outname = "{initial_edgefile}_{clustnum}.net".format(
                initial_edgefile=initial_edgefile, clustnum=str(i)
            )
            infomap_outfile = "{initial_edgefile}_{clustnum}.tree".format(
                initial_edgefile=initial_edgefile, clustnum=str(i)
            )
            ...
            # Runs infomap
            cmd = "{infomap} -i pajek {infomap_args} {pajek_file} infomap_out/".format(
                infomap=infomap_path, pajek_file=outname, infomap_args=infomap_args
            )
            os.system(cmd)
...
def make_edgefile(
    ...
    trn_table["Negative selection cutoff"] = linear_model.get_prediction(
        predict_df
    ).summary_frame(alpha=0.1)["obs_ci_upper"]
    ...
    keep_edges = trn_table[
        (~trn_table["Strain 1"].isin(flat_clones))
        & (~trn_table["Strain 2"].isin(flat_clones))
    ]
    keep_edges = zip(
        keep_edges["Strain 1"], keep_edges["Strain 2"], keep_edges["Observed SSD"]
    )
